...
@section('content_header')
    <div class="container">
        <div class="row p-2 align-items-center justify-content-between d-flex">
            <h1 class="text-bold">Kontrak Mata Kuliah</h1>
            <x-adminlte-button label="Tambah Kontrak Mata kuliah" data-toggle="modal" data-target="#modalCustom"
...
@section('content')
    <div class="container">
            ...
            <tbody class="table-group-dividerr">
                @foreach ($kontrak as $data)
                    <tr>
                        ...
                        <td scope="col">{{ $data->semester->semester }}</td>
                    ...
                    {{-- Modal Edit --}}
                    <x-adminlte-modal id="modalCustomEdit{{ $data->id }}" title="Edit Kontrak Mata Kuliah"
                        ...
                        <div class="form-group">
                            <form action="/kontrak/{{ $data->id }}" method="post">
                                ...
                                <x-adminlte-select2 name="mahasiswa_id">
                                    ...
                                    <option selected value="{{ $data->mahasiswa->id }}">
                                        {{ $data->mahasiswa->nama_mahasiswa }}
                                    </option>

                                    @foreach ($mahasiswa as $datamahasiswa)
                                        <option value="{{ $datamahasiswa->id }}">{{ $datamahasiswa->nama_mahasiswa }}
                                        </option>
                                    @endforeach
                                </x-adminlte-select2>

                                <label for="semester_id">Semester :</label>
                                <x-adminlte-select2 name="semester_id">
                                    <option selected value="{{ $data->semester->id }}">
                                        {{ $data->semester->semester }}</option>
        ...
        <div>{{ $kontrak->links() }}</div>

        {{-- Modal Tambah --}}
        <x-adminlte-modal id="modalCustom" title="Tambah Kontrak Mata Kuliah" size="md" theme="primary" v-centered
